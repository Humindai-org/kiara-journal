...
function relativeTime(dateStr: string) {
  ...
  if (h < 24)
    return `Today, ${new Date(dateStr).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}`;
...
function OverviewItem({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-sm transition-colors active:scale-95",
        focusRing,
        active
    ...
    >
      ...
      <span
        className={cn(
          ...
          active ? "bg-accent/20 text-accent" : "bg-surface-2 text-text-disabled"
        )}
      >
        {count}
      </span>
...
export default function NotebookPage() {
  ...
  // Date.now() is impure during render (react-hooks/purity); captured when notes load
  const [now, setNow] = useState(0);

  const saveTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const editorRef = useRef<{ noteId: string; title: string; content: string; tags: string[] } | null>(null);
  ...
  const counts = useMemo(() => {
    ...
    return {
      ...
      recent: notes.filter((n) => new Date(n.updated_at).getTime() > recentCutoff).length,
  ...
  const displayedNotes = useMemo(() => {
    ...
    if (activeFolder === "FAVORITES") filtered = filtered.filter((n) => n.is_favorite);
      ...
      const def = DEFAULT_FOLDERS.find((f) => f.name === activeFolder);
      filtered = filtered.filter(
        (n) => n.folder === activeFolder || (def && n.category === def.category)
      );
    }
    ...
    return [...filtered].sort((a, b) => Number(b.is_pinned) - Number(a.is_pinned));
  ...
  const categoryDistribution = useMemo(() => {
    const dist = (Object.keys(CATEGORY_META) as Category[])
  ...
  const donutGradient = useMemo(() => {
    ...
    let acc = 0;
    const stops = categoryDistribution.map((c) => {
  ...
  const streakData = useMemo(() => {
    ...
    for (const n of notes) {
      activeDays.add(dayKey(new Date(n.created_at)));
      activeDays.add(dayKey(new Date(n.updated_at)));
    }
    let streak = 0;
    const cursor = new Date();
    // A streak can survive not having journaled yet today
    ...
    const week = WEEKDAYS.map((label, i) => {
      ...
      return { label, done: activeDays.has(dayKey(d)), future: d.getTime() > today.getTime() };
    });
    return { streak, week };
  }, [notes]);
  ...
  const saveNote = useCallback(async () => {
    ...
    if (!snapshot) return;
    setSaving(true);
    const title = snapshot.title.trim() || "Untitled Note";
    const updated_at = new Date().toISOString();
    const { error } = await db
      .from("notebooks")
      .update({ title, content: snapshot.content, tags: snapshot.tags, updated_at })
      .eq("id", snapshot.noteId);
    setSaving(false);
    if (error) {
    ...
    setNotes((prev) =>
      prev.map((n) =>
        n.id === snapshot.noteId
          ? { ...n, title, content: snapshot.content, tags: snapshot.tags, updated_at }
          : n
      )
    );
    setSavedAt(new Date());
  ...
  // Keep a snapshot for debounced/shortcut saves without stale closures
  useEffect(() => {
    editorRef.current = selectedNote
      ? { noteId: selectedNote.id, title: editTitle, content: editContent, tags: editTags }
      : null;
  }, [selectedNote, editTitle, editContent, editTags]);
  ...
  async function toggleFavorite(noteId: string) {
    ...
    const { error } = await db.from("notebooks").update({ is_favorite: newVal }).eq("id", noteId);
    if (error) {
  ...
  async function togglePin(noteId: string) {
    ...
    const { error } = await db.from("notebooks").update({ is_pinned: newVal }).eq("id", noteId);
    if (error) {
      toast.error("Error updating pin");
      return;
    }
    setNotes((prev) => prev.map((n) => (n.id === noteId ? { ...n, is_pinned: newVal } : n)));
    setOpenMenuId(null);
  }
  ...
  async function moveToFolder(noteId: string, folderName: string) {
    ...
    setNotes((prev) =>
      prev.map((n) =>
        n.id === noteId ? { ...n, folder: folderName, ...(def ? { category: def.category } : {}) } : n
      )
    );
    setOpenMenuId(null);
    setMoveMenuId(null);
      ...
      <main className="flex-1 overflow-hidden flex">
        ...
        <aside className="w-56 shrink-0 bg-sidebar border-r border-border flex flex-col overflow-y-auto">
          <div className="p-3 space-y-5">
            <div>
              <p className="text-[10px] text-text-muted uppercase tracking-wider px-2.5 mb-1.5">
                Overview
              </p>
              <div className="space-y-0.5">
                ...
                <OverviewItem
                  ...
                  onClick={() => selectOverview("FAVORITES")}
                />
                <OverviewItem
                  icon={Clock}
                  label="Recent"
                ...
                {DEFAULT_FOLDERS.map((f) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "relative w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-sm transition-colors active:scale-95",
                        focusRing,
                        active
            ...
            <div>
              ...
              <button
                onClick={createNote}
                className={cn(
                  "btn-action w-full flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm font-medium transition-opacity hover:opacity-90 active:scale-95",
                  focusRing
                )}
              >
              ...
              <div className="flex gap-1.5 mt-2">
                <button
                  ...
                  className={cn(
                    "flex-1 px-2 py-1.5 rounded-lg text-xs text-text-muted border border-border hover:text-text-primary hover:bg-surface-2 transition-colors active:scale-95",
                    focusRing
                  )}
                >
                  Daily Journal
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex items-center gap-3 px-6 py-4 border-b border-border">
                <button
                  onClick={closeEditor}
                  className={cn(
                    "flex items-center gap-1 text-sm text-text-secondary hover:text-text-primary transition-colors active:scale-95 rounded-lg px-2 py-1",
                    focusRing
                  )}
                >
                ...
                <button
                  ...
                  className={cn(
                    "btn-action px-4 py-1.5 rounded-lg text-sm font-medium transition-opacity hover:opacity-90 active:scale-95",
                    focusRing
                  )}
                >
                  Save
              ...
              <div className="flex-1 flex flex-col overflow-y-auto px-8 py-6 max-w-3xl w-full mx-auto">
                <div className="flex flex-wrap gap-1.5 mb-5">
                  {(Object.keys(CATEGORY_META) as Category[]).map((c) => {
                    const meta = CATEGORY_META[c];
                    const Icon = meta.icon;
                    const active = selectedNote.category === c;
                    return (
                      <button
                        key={c}
                        onClick={() => changeCategory(c)}
                        className={cn(
                          "flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs border transition-colors active:scale-95",
                          focusRing,
                          active
                            ? "text-text-primary"
                            : "border-border text-text-disabled hover:text-text-secondary hover:bg-surface-2"
                        )}
                        style={
                          active
                ...
                <input
                  ...
                  className="w-full bg-transparent text-2xl font-bold tracking-tight text-text-primary placeholder:text-text-disabled focus:outline-none"
                />
                ...
                <textarea
                  ...
                  className="flex-1 min-h-[320px] w-full resize-none bg-transparent text-sm leading-relaxed text-text-secondary placeholder:text-text-disabled focus:outline-none"
                />
              </div>

              <div className="flex items-center gap-4 px-8 py-3 border-t border-border text-xs text-text-disabled">
                <div className="flex items-center gap-1.5 flex-wrap flex-1 min-w-0">
                  {editTags.map((tag) => (
                    <span
                      key={tag}
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-accent-soft text-accent"
                    >
                      #{tag}
                      <button
                        onClick={() => removeTag(tag)}
                        className={cn("hover:text-text-primary transition-colors", focusRing, "rounded-full")}
                ...
                <span className="tabular-nums shrink-0">{wordCount} words</span>
              ...
              <div className="sticky top-0 z-10 bg-bg px-6 py-4 border-b border-border space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={cn(
                        "w-48 bg-surface-hi hover:bg-surface-hover rounded-lg pl-8 pr-3 py-1.5 text-xs text-text-primary placeholder:text-text-disabled border border-border transition-colors",
                        focusRing
                      )}
                    />
                  </div>
                  <div className="flex rounded-lg border border-border overflow-hidden">
                    <button
                      ...
                      className={cn(
                        "size-8 flex items-center justify-center transition-colors active:scale-95",
                        focusRing,
                        viewMode === "grid"
                          ? "bg-accent-soft text-accent"
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto [scrollbar-width:none]">
                  {CATEGORY_TABS.map((tab) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "px-3 py-1 rounded-full text-xs border whitespace-nowrap transition-colors active:scale-95",
                          focusRing,
                          active
                  ...
                  <button
                    ...
                    aria-label="New note"
                    className={cn(
                      "size-6 shrink-0 flex items-center justify-center rounded-full border border-border text-text-disabled hover:text-text-secondary hover:bg-surface-2 transition-colors active:scale-95",
                      focusRing
                    )}
                  >
                ...
                {loading ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center gap-3 text-center px-6">
                    ...
                    <p className="text-xs text-text-muted max-w-56">
                      Capture strategies, mindset notes, routines and templates in one place.
                    </p>
                    <button
                      onClick={createNote}
                      className={cn(
                        "btn-action mt-1 px-4 py-2 rounded-lg text-sm font-medium transition-opacity hover:opacity-90 active:scale-95",
                        focusRing
                      )}
                    >
                    ...
                    {displayedNotes.map((note) => {
                      ...
                      return (
                        <div
                          ...
                          onKeyDown={(e) => e.key === "Enter" && openEditor(note)}
                          className={cn(
                            "card relative p-4 pt-5 cursor-pointer overflow-hidden group hover:bg-surface-2 transition-colors",
                            focusRing
                          )}
                        >
                          <span
                            className="absolute inset-x-0 top-0 h-[3px]"
                            style={{ backgroundColor: note.color ?? meta.color }}
                          />
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <div className="relative">
                              <button
                                ...
                                className={cn(
                                  "size-6 flex items-center justify-center rounded-md text-text-disabled hover:text-text-primary hover:bg-surface-hi transition-colors active:scale-95",
                                  focusRing
                                )}
                              >
                              ...
                              {openMenuId === note.id && (
                                <div
                                  ...
                                  className="absolute right-0 top-7 z-20 w-44 rounded-xl bg-surface-light border border-border-light shadow-[0_2px_10px_rgba(0,0,0,0.15),_0_12px_40px_rgba(0,0,0,0.20)] p-1"
                                >
                                  <button
                                    onClick={() => duplicateNote(note.id)}
                                    className={cn(
                                      "w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs text-text-secondary hover:text-text-primary hover:bg-surface-2 transition-colors",
                                      focusRing
                                    )}
                                  >
                                  ...
                                  <button
                                    onClick={() => setMoveMenuId(moveMenuId === note.id ? null : note.id)}
                                    className={cn(
                                      "w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs text-text-secondary hover:text-text-primary hover:bg-surface-2 transition-colors",
                                      focusRing
                                    )}
                                  ...
                                  {moveMenuId === note.id &&
                                    DEFAULT_FOLDERS.map((f) => (
                                      <button
                                        key={f.name}
                                        onClick={() => moveToFolder(note.id, f.name)}
                                        className={cn(
                                          "w-full flex items-center gap-2 pl-7 pr-2.5 py-1.5 rounded-lg text-xs text-text-muted hover:text-text-primary hover:bg-surface-2 transition-colors",
                                          focusRing
                                        )}
                                      >
                                        {f.name}
                                  ...
                                  <button
                                    ...
                                    className={cn(
                                      "w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-xs text-loss hover:bg-loss/10 transition-colors",
                                      focusRing
                                    )}
                                  >
                            ...
                            <button
                              ...
                              className={cn(
                                "size-6 flex items-center justify-center rounded-md transition-colors active:scale-95",
                                focusRing,
                                note.is_favorite
                                  ? "text-warning"
                          ...
                          <h3 className="text-sm font-bold tracking-tight text-text-primary mb-1 truncate">
                            {note.title}
                          </h3>
                          <p className="text-xs leading-relaxed text-text-secondary line-clamp-2 min-h-8">
                    ...
                    {displayedNotes.map((note) => {
                      ...
                      return (
                        <div
                          ...
                          onKeyDown={(e) => e.key === "Enter" && openEditor(note)}
                          className={cn(
                            "grid grid-cols-[28px_1fr_100px_120px_50px_70px] gap-3 items-center px-3 py-2.5 rounded-xl bg-surface border border-border cursor-pointer hover:bg-surface-2 transition-colors",
                            focusRing
                          )}
                        >
                          <span
                          ...
                          <span
                            className="text-[10px] font-medium uppercase tracking-wider px-2 py-0.5 rounded-full w-fit"
                            style={{ backgroundColor: `${meta.color}1f`, color: meta.color }}
                          >
                            {meta.label}
                          </span>
                          ...
                          <span className="flex items-center justify-end gap-1">
                            ...
                            <button
                              ...
                              className={cn(
                                "size-6 flex items-center justify-center rounded-md text-text-disabled hover:text-loss transition-colors active:scale-95",
                                focusRing
                              )}
                            >
          ...
          <div className="card p-5">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div>
                ...
                <p className="text-[10px] text-text-muted mt-0.5">Total Notes</p>
              </div>
              <div>
                <p className="text-2xl font-bold tracking-tight text-text-primary tabular-nums">
                  {totalFolders}
                </p>
                <p className="text-[10px] text-text-muted mt-0.5">Folders</p>
              </div>
              <div>
                <p className="text-2xl font-bold tracking-tight text-accent tabular-nums">
                  {counts.pinned}
                </p>
                <p className="text-[10px] text-text-muted mt-0.5">Pinned</p>
              </div>
            </div>
          </div>

          <div className="card p-4 space-y-2">
            <p className="text-xs font-medium text-text-muted mb-1">Recent Activity</p>
              ...
              recentActivity.map((note) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => openEditor(note)}
                    className={cn(
                      "w-full flex items-center gap-2 px-1.5 py-1.5 rounded-lg hover:bg-surface-2 transition-colors active:scale-95",
                      focusRing
                    )}
                  >
                    <span
            ...
            <button
              ...
              className={cn(
                "w-full text-left text-[10px] text-accent hover:text-accent-dim transition-colors px-1.5 pt-1 rounded",
                focusRing
              )}
            >
              View all activity
          ...
          <div className="card p-4">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-lg font-bold tracking-tight text-text-primary">
                ...
                <span className="text-xs font-normal text-text-muted">in a row</span>
              </p>
            </div>
